...
const IdeaBucketPage: React.FC = () => {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const data = await apiService.get<BucketItem[]>(
        `/trips/${tripId}/bucketItems`,
      );
      setItems(data);
    } catch { /* ignore */ }
  }, [apiService, tripId]);
  ...
  const handleAdd = async (
    ...
    try {
      ...
      if (locationRef.current) {
        ...
        if (!payload.location) {
          payload.location = locationRef.current.label.split(",")[0];
        }
      }
      const newItem = await apiService.post<BucketItem>(
        `/trips/${tripId}/bucketItems`,
        payload,
      );
      ...
      notification.success({
        title: "Bucket item created successfully!",
        placement: "bottomRight",
        className: "custom-notification",
      });
    } catch (e) {
      message.error((e as Error).message ?? "Failed to add idea");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEdit = async (
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((i) => i.bucketItemId === updated.bucketItemId ? updated : i)
      );
      setEditingItem(null);
      ...
      message.error((e as Error).message ?? "Failed to update idea");
    }
  };
  ...
  const handleDelete = async (itemId: number) => {
    try {
      ...
      message.error((e as Error).message ?? "Failed to delete idea");
    }
  };
  ...
  const handleVote = async (item: BucketItem, value: number) => {
    ...
    try {
      const updated = await apiService.post<BucketItem>(
        `/trips/${tripId}/bucketItems/${item.bucketItemId}/vote`,
        { value: newValue },
      );
      setItems((prev) =>
        prev.map((i) => i.bucketItemId === updated.bucketItemId ? updated : i)
      ...
      message.error((e as Error).message ?? "Failed to vote");
    }
  };

  const handleSchedule = async (
    values: { date: Dayjs; startTime: string; endTime: string },
  ) => {
    ...
    try {
      ...
      message.error((e as Error).message ?? "Failed to schedule");
    }
  };
  ...
  return (
    <div style={{ padding: "16px 0" }}>
      ...
      >
        <div>
          <Title level={3} style={{ margin: 0, color: "#111" }}>
            Idea Bucket
          </Title>
                  ...
                  {item.addedBy.id === Number(currentUserId) && (
                    <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                      ...
                      <Popconfirm
                        ...
                        onConfirm={() =>
                          handleDelete(item.bucketItemId)}
                        okText="Yes"
                        cancelText="No"
                      >
                        <Button
