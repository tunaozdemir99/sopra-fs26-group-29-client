...
const TasksPage: React.FC = () => {
  ...
  const handleAdd = async (
    values: { title: string; description?: string; assigneeId: number },
  ) => {
    setSubmitting(true);
    try {
      ...
      message.error((e as Error).message ?? "Failed to add task");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdateStatus = async (taskId: number, status: TaskStatus) => {
    try {
      const updated = await apiService.patch<Task>(
      ...
      message.error((e as Error).message ?? "Failed to update task");
    }
  };
  ...
  const handleDelete = async (taskId: number) => {
    try {
      ...
      message.error((e as Error).message ?? "Failed to delete task");
    }
  };

  const openModal = async () => {
    try {
      const members = await apiService.get<{ id: number; username: string }[]>(
    ...
    <div style={{ padding: "16px 0" }}>
      ...
      >
        <div>
          ...
          <Text type="secondary" style={{ fontSize: 13 }}>
            Organize and track tasks for your trip
          </Text>
        </div>
        <Button
          type="primary"
      ...
      {tasks.length === 0 && (
        <div style={{ textAlign: "center", padding: "64px 0" }}>
          ...
          <Text type="secondary" style={{ fontSize: 13 }}>
            Create tasks to organize your trip planning
          </Text>
        </div>
      )}
                          ...
                          >
                            ...
                            >
                              ...
                              <Select
                                ...
                                onChange={(val) =>
                                  handleUpdateStatus(task.taskId, val)}
                                optionRender={(opt) => (
